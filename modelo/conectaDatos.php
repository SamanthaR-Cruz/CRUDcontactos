...
class conectaDatos {
	...
	function ejecutarConsulta($psConsulta) {
		...
		if ($psConsulta == "") {
			throw new Exception("conectaDatos->ejecutarConsulta: falta indicar la consulta");
		}
		if ($this->oConexion == null) {
			throw new Exception("conectaDatos->ejecutarConsulta: falta conectar la base");
		}
		try {
	...
	function ejecutarComando($psComando) {
		$nAfectados = -1;
		if ($psComando == "") {
			throw new Exception("conectaDatos->ejecutarComando: falta indicar el comando");
		}
		if ($this->oConexion == null) {
			throw new Exception("conectaDatos->ejecutarComando: falta conectar la base");
		}
		try {
